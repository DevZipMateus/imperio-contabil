...
const ServicesSection = () => {
  ...
  const features = ["Atendimento personalizado", "Suporte contínuo", "Tecnologia de ponta", "Equipe especializada", "Conformidade garantida", "Resultados comprovados"];
  ...
  return <section id="services" ref={sectionRef} className="section-padding bg-white">
        ...
        <div className="bg-gradient-to-r from-primary/5 to-accent/5 rounded-2xl p-8 md:p-12 animate-on-scroll">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <Button size="lg" onClick={handleWhatsAppClick} className="bg-primary hover:bg-primary/90 text-accent-foreground font-semibold group">
                Fale Conosco Agora
            ...
            <div className="relative">
              ...
              <img src="/lovable-uploads/dacc99de-aa0c-4c37-ad6f-7a225d124757.png" alt="Império Contabilidade" className="relative z-10 w-full max-w-md mx-auto object-contain" />
            </div>
          </div>
        </div>
      </div>
...
const ServiceCard = ({
  ...
  description,
  index
}: ServiceCardProps) => <Card className={`animate-on-scroll service-card border border-primary/10 shadow-md hover:shadow-lg transition-all duration-300 h-full [animation-delay:${index * 100}ms]`}>
    <CardContent className="p-8 flex flex-col items-center text-center h-full">
      <div className="mb-6 p-4 bg-primary/10 rounded-full">{icon}</div>
      <h3 className="text-xl font-bold mb-4 text-gray-50">{title}</h3>
      <p className="leading-relaxed flex-grow text-gray-50">{description}</p>
    </CardContent>
  </Card>;
