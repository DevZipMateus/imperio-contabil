...
const TestimonialsSection = () => {
  ...
  return <section ref={sectionRef} className="section-padding bg-gray-50 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4">
          <div className="animate-on-scroll">
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 testimonial-slider transform`}>
              {getVisibleTestimonials().map((testimonial, index) => <TestimonialCard key={index} testimonial={testimonial} />)}
            </div>
          </div>

          <div className="flex justify-center mt-10 space-x-4">
            <button onClick={handlePrev} className="p-3 rounded-full bg-white shadow-md border border-gray-200 text-gray-900 hover:bg-gray-50 transition-colors" aria-label="Depoimento anterior">
              <ChevronLeft size={20} />
            </button>
            ...
            <div className="flex space-x-2">
              ...
              setTimeout(() => setIsTransitioning(false), 600);
            }} className={`w-3 h-3 rounded-full transition-colors ${currentIndex === index ? 'bg-primary' : 'bg-gray-300'}`} aria-label={`Ir para depoimento ${index + 1}`} />)}
            </div>
            
            <button onClick={handleNext} className="p-3 rounded-full bg-white shadow-md border border-gray-200 text-gray-900 hover:bg-gray-50 transition-colors" aria-label="Próximo depoimento">
...
const TestimonialCard = ({
  testimonial
    ...
    <CardContent className="p-8 h-full flex flex-col">
      ...
      <p className="mb-6 flex-grow text-gray-50">{testimonial.text}</p>
      ...
      <div className="flex items-center">
        ...
        <div>
          ...
          <p className="text-sm text-gray-50">{testimonial.company}</p>
        </div>
      </div>
    </CardContent>
  </Card>;
